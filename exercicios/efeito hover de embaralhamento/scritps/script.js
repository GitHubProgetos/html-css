...
function initGridOverlay(element) {
    ...
    for (let i = 0; i < clusterCount; i++) {
        const neighbors = blocks.filter((neighbor) => {
            ...
            // Calcula a distância na grade (grid) para achar vizinhos diretos
            const dx = Math.abs(neighbor.gridX - currentBlock.gridX);
            const dy = Math.abs(neighbor.gridY - currentBlock.gridY);

            // Retorna verdadeiro se o bloco estiver colado (horizontal, vertical ou diagonal)
            return dx <= 1 && dy <= 1;
        });
            if (neighbors.length === 0) break;

            // Escolhe um vizinho aleatório para expandir o efeito
 ...
 initGridOverlay(document.querySelector(".hover-img"));
